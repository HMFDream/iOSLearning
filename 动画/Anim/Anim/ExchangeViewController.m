...
//    [self.view addSubview:self.btnPush];
    ...
    NSArray *titleArr = @[@"Fade",@"MoveIn",@"Push",@"Reveal",@"pageCurl",@"pageUnCurl",@"rippleEffect",@"suckEffect",@"cube",@"oglFlip"];
    
    for (int i=0; i<5; i++) {
        UIButton *btn1 = [[UIButton alloc]initWithFrame:CGRectMake(0+i*70, 100, 50, 30)];
        [btn1 setTitle:titleArr[i] forState:UIControlStateNormal];
        [btn1 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [btn1 setTag:i+1000];
        [btn1 addTarget:self action:@selector(exchangeView:) forControlEvents:UIControlEventTouchUpInside];
    ...
    for (int i=5; i<10; i++) {
        UIButton *btn1 = [[UIButton alloc]initWithFrame:CGRectMake((i-5)*70, 150, 50, 30)];
        [btn1 setTitle:titleArr[i] forState:UIControlStateNormal];
        [btn1 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [btn1 setTag:i+1000];
        [btn1 addTarget:self action:@selector(exchangeView:) forControlEvents:UIControlEventTouchUpInside];
